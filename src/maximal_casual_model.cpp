...
class MaximalCasualModel {
   ...
   public:
    ...
    void generateMHBConstraints() {
        for (const auto& [threadId, events] : trace.getThreadToEventsMap()) {
            for (int i = 0; i < events.size(); ++i) {
                std::shared_ptr<Expr> eventExpr = ExprBuilder::var(events[i].getEventId());
                ...
                if (i == 0) {
                    ...
                    std::shared_ptr<Expr> prevEventExpr = ExprBuilder::var(events[i - 1].getEventId());
    ...
    void generateLockConstraints() {
        for (const auto& [lockId, acq_rel_pairs] : trace.getLockPairs()) {
            for (int i = 0; i < acq_rel_pairs.size(); ++i) {
                std::shared_ptr<Expr> acq1 = ExprBuilder::var(acq_rel_pairs[i].first.getEventId());
                std::shared_ptr<Expr> rel1 = ExprBuilder::var(acq_rel_pairs[i].second.getEventId());
                for (int j = i + 1; j < acq_rel_pairs.size(); ++j) {
                    if (acq_rel_pairs[i].first.getThreadId() ==
                        acq_rel_pairs[j].first.getThreadId())
                        continue;
                    std::shared_ptr<Expr> acq2 = ExprBuilder::var(acq_rel_pairs[j].first.getEventId());
                    std::shared_ptr<Expr> rel2 = ExprBuilder::var(acq_rel_pairs[j].second.getEventId());
        ...
        */
        ...
        if (same_initial_val && !feasibleWrites.empty()) {
            ...
            for (Event& ifw : inFeasibleWrites) {
                std::shared_ptr<Expr> ifw_expr = ExprBuilder::var(ifw.getEventId());
                if (tmp_expr == nullptr) {
                    tmp_expr = ExprBuilder::lessThan(ifw_expr, r_expr);
                } else {
                    tmp_expr = ExprBuilder::andExpr(tmp_expr, ExprBuilder::lessThan(r_expr, ifw_expr));
                }
            }
            exp = tmp_expr;
        }
    ...
    std::pair<z3::expr, ConstraintStatus> getWriteCFConstraints(Event w, z3::context& c) {
        ...
        for (Event& e : trace.getEventsFromThread(w.getThreadId())) {
            ...
            if (e.getEventType() != Event::EventType::Read)
                continue;
            if (cf_constraints.find(e.getEventId()) != cf_constraints.end() && generating_cf[e.getEventId()]) {
    ...
    std::pair<z3::expr, ConstraintStatus> getReadCFConstraints(Event r, z3::context& c) {
        if (generating_cf.find(r.getEventId()) != generating_cf.end() && generating_cf[r.getEventId()]) {
            return {z3::expr(c), ConstraintStatus::NotPossible};
        }
    ...
    void generateAllCFConstraints(z3::context& c, z3::solver& s) {
        for (auto [threadId, events] : trace.getThreadToEventsMap()) {
            for (Event& r : events) {
                if (r.getEventType() != Event::EventType::Read)
                    continue;
                std::vector<Event> feasibleWrites;
                std::vector<Event> inFeasibleWrites;
                bool same_initial_val = r.getVarValue() == trace.getReadEvents(r.getVarId())[0].getVarValue();
    ...
    void generateReadCFConstraints() {
        for (auto [threadId, events] : trace.getThreadToEventsMap()) {
            for (Event& r : events) {
                ...
                DEBUG_PRINT("Generating read cf constraints for event " << r.getEventId());
                std::vector<Event> feasibleWrites;
                std::vector<Event> inFeasibleWrites;
                bool has_write_events = trace.getWriteEvents(r.getVarId()).size() > 0;
                bool same_initial_val = 
                    r.getVarValue() == trace.getReadEvents(r.getVarId())[0].getVarValue() 
                    && (!has_write_events || trace.getReadEvents(r.getVarId())[0].getEventId() < trace.getWriteEvents(r.getVarId())[0].getEventId());
                ...
                DEBUG_PRINT("Feasible writes: " << feasibleWrites.size());
                DEBUG_PRINT("Infeasible writes: " << inFeasibleWrites.size());
                DEBUG_PRINT("###############################################");
            ...
            DEBUG_PRINT("read cf constraints generated for thread " << threadId);
        }
    ...
    uint32_t solveForRace() {
        ...
        for (auto cop : trace.getCOPEvents()) {
            DEBUG_PRINT("Solving for cop " << cop.first.getEventId() << ", " << cop.second.getEventId());
        ...
        for (const auto& cop : trace.getCOPEvents()) {
            #ifdef DEBUG
            std::cout << "Solving for cop " << cop.first.getEventId() << ", " << cop.second.getEventId() << "\n";
            #endif
            ...
            //     std::cout << "no of cf1 constraints: " << cf1Visitor.getCount() << "\n";
            // }

            // if (cf2 != nullptr) {
            //     ConstraintCounter cf2Visitor;
            //     cf2->accept(cf2Visitor);
            //     std::cout << "no of cf2 constraints: " << cf2Visitor.getCount() << "\n";
            // }
