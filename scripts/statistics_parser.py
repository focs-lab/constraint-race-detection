...
def process_block(block):
    ...
    # patterns = {
    #     "name": r"Processing:\s*(\w+)",
    #     "single_cop_check_time": r"Total time taken: (\d+)ms",
    #     "single_cop_check_memory_used": r"Maximum resident set size \(kbytes\):\s*(\d+)",
    # }
    patterns = {
        "name": r"Processing:\s*(\w+)",
        "no_of_races_detected": r"Number of races detected: (\d+)",
        "total_time": r"Total time taken: (\d+)ms",
    }
...
def process_file(file_path, output_csv):
    ...
    with open(output_csv, 'w', newline='') as csvfile:
        ...
        # ]
        fieldnames = [
            "name", "no_of_races_detected", "total_time"
        ]
